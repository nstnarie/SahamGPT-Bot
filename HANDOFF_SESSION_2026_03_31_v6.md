...
> Last updated: March 31, 2026 (end of session — Q3 2025 broker data complete, Q4 ready to start)
...
## 6. Database State (as of 2026-03-31 end of session)
...
- **Current record count:** 756,370
- **Date range in DB:** 2025-01-02 to 2025-09-30
...
| Jul–Sep 2025 (Q3) | ✅ Complete | 64 days, 105–107 tickers/day, zero low days |
| Oct–Dec 2025 (Q4) | ❌ Not started | Next priority |
| 2024 full year | ❌ Not started | After H2 + backtest confirms improvement |

### Q3 2025 scraping detail
| Batch | Delta | Running Total |
|-------|-------|---------------|
| Baseline (H1 end) | — | 468,484 |
| B1 Jul–Sep | +84,313 | 552,797 |
| B2 Jul–Sep | +70,317 | 623,128 |
| B3 Jul–Sep | +58,191 | 681,319 |
| B4 Jul–Aug15 | +38,676 | 719,995 |
| B4 Aug15–Sep | +36,375 | 756,370 |

Note: Batch 4 was split into 2 date-range parts (Jul 1–Aug 15, Aug 15–Sep 30) to stay within GitHub Actions 6-hour timeout. Duplicate guard handles the Aug 15 overlap safely.

### Baseline artifact & split files
- **756,370 records** is the current baseline
- Split files updated and committed to repo root via `update_split_files.yml` after Q3 completion
- Split files are used as fallback if artifact download fails
...
- **Artifact download** uses `dawidd6/action-download-artifact@v6` — NOT `actions/download-artifact@v4` (v4 only downloads from current run, not previous runs)
...
- **Batch 4 (34 tickers) for a full quarter (~65 days) takes ~5.5 hours** — split into 2 date-range parts to stay safe within 6-hour timeout
...
### Q4 2025 Scraping (Oct–Dec) — NEXT UP
Run all 4 batches sequentially. **Refresh Stockbit token before starting each session.**

```
batch: 1  |  start_date: 2025-10-01  |  end_date: 2025-12-31
batch: 2  |  start_date: 2025-10-01  |  end_date: 2025-12-31
batch: 3  |  start_date: 2025-10-01  |  end_date: 2025-12-31
batch: 4  |  start_date: 2025-10-01  |  end_date: 2025-11-15   ← part 1
batch: 4  |  start_date: 2025-11-15  |  end_date: 2025-12-31   ← part 2
```

After all batches: run `export_summary.yml` (2025-10-01 to 2025-12-31) to verify, then `update_split_files.yml`.

### Backtesting (PRIORITY — before 2024 scraping)
1. **Re-run backtests with 2025 real broker data** — replace synthetic foreign flow with real Asing `net_value` from `broker_summary` table
2. **Evaluate results** — if 2025 backtest shows significant profit improvement, proceed to 2024 scraping. Otherwise, tune signal logic first.

### 2024 Backfill (only after 2025 backtest confirms improvement)
3. Backfill 2024 full year — all 4 batches, split into quarterly chunks
4. Re-run backtests with 2024 + 2025 real broker data

### Integration & improvement
5. Integrate real broker data into `signal_combiner.py` — replace synthetic foreign flow with real Asing `net_value` from `broker_summary` table
6. Fix 6–10 day stop-loss weak spot — 54 trades, 8% WR, -215M loss in this range. Use broker accumulation signal to decide whether to hold or exit
7. Update `daily_signals.yml` to include live broker scraping each day
8. Paper trade 1 month → go live
